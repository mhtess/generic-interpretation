...
\subsection{Application to controversial generics}

We briefly discuss how our model understands two pairs of problematic or controversial generic statements, for which the interpretation can change depending on the speaker or listener. Consider the following utterance and response:
\begin{quotation}
\noindent A: \emph{Men are pigs.}\\ 
B: \emph{Not all men.}
\end{quotation}

A utters a generic statement, which our model understands as communicating some underspecified prevalence (in this case, of piggishness among men). According to our \emph{uncertain threshold} model, the interpretation of 100\% prevalence (i.e., \emph{all men are pigs}) is always on the table.
Given that it is a possibility (among many) that the model considers, B may hear A as implying that all men are pigs. 
Then B may feel justified in responding that \emph{Not all men are pigs}. 
One could imagine the dialogue continuing with A saying \emph{I didn't say all men are pigs}, and the two engaging a meta-discourse about the meaning of generics.

Consider another controversial pair of statements:
\begin{quotation}
\noindent A: \emph{Black lives matter.}\\ 
B: \emph{All lives matter.}
\end{quotation}

This pair highlights another dimension that our model of generic language elucidates. 
It is straightforward to understand that A is uttering that \emph{Black lives matter} to affirm, in frustration to the facts that Black lives are currently treated as less precious, that Black lives are indeed precious.\footnote{This example also hints at the difference between empirical frequency (i.e., the facts on the ground) and predictive prevalence (e.g., what we expect or hope the future the be). For an empirical demonstration of the centrality of predictive prevalence, see  \citeNP{Tessler2019psychrev}, Case Study 2).} 
% both the belief that Black lives should matter as much as other lives and 
A generic can be taken to communicate about the prevalence within a category (as A is intending), but because the interpretation appeals to prior beliefs about prevalence within a comparison class of alternative categories, a generic can also communicate about the distinctiveness of the feature for the category relative to other categories, what is otherwise called \emph{cue validity} (see \citeNP{Tessler2019psychrev} for a detailed discussion of this relationship). Thus, one available interpretation of A's statement is that \emph{Black lives, more so than other lives, matter}. This interpretation is presumably what B is responding to, when they say \emph{All lives matter}. 
The distinction between whether the generic is explicitly being deployed in a contrastive or a non-contrastive manner depends more generally on the Question Under Discussion \cite{degenBLM}, which in our model would modulate the comparison class (i.e., the set of alternative categories that the listener has in mind).

More broadly, our model highlights that precise communication requires prior beliefs that are known to each interlocutor (i.e., the speaker must have an accurate representation of the listener's prior beliefs). Beliefs are, however, unobservable and it is unlikely that interlocutors will ever perfectly align on their prior beliefs.
Miscommunication will result when these priors become too misaligned, which each of the examples above highlight. 
