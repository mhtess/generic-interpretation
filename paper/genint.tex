...
Recently, Tessler and Goodman (2019) proposed a model for a meaning of generics as a vague quantifier, which predicted truth judgments (or, endorsements) of generics with high quantitative accuracy. 
...
The vague quantifier model of generics predicts strong dependence of interpretation on prior beliefs about the prevalence of features. Across two experiments, we both measure and manipulate prior beliefs. We find that prior beliefs are causally related to generic interpretations, as predicted, and that generics are best modeled as a vague quantifier.
...
The most basic dimension of belief-updating about categories and properties is the prevalence of a feature in a category, or the probability that an instance $x$ of the category $k$ has feature $f$: $P(f(x){=}\texttt{true} \mid x\in k)$, or $P(f \mid k$) for short-hand.
In this regard, a common intuition with generics is that they \enquote{{[}\ldots{}{]} appear to be commonly taken in a rather strong sense, as though the qualifier \emph{always} had implicitly crept into their interpretation} \cite<p.~172,>{Abelson1966}. 
Indeed, adults learning from generics about both familiar and novel animals (e.g., \enquote{Bears like to eat ants}; \enquote{Lorches have purple feathers}) rate the prevalence of the feature (e.g., the percentage of bears that like to eat ants) implied by the generic as very high \cite<mean implied prevalence $\sim 85\%$;>{Gelman2002, Cimpian2010}.
It is not enough to say, however, that that meaning of a generic $Ks F$ is that $P(f \mid k)$ is high. 
...
Some generics (e.g., \enquote{Mosquitos carry malaria}) seem to only convey that the prevalence is non-zero, an existential claim, as is the qualifier ``can'' had implicitly crept in: \emph{mosquitos can carry malaria}.
...
The model --- originally proposed by \citeA{Tessler2019psychrev} --- takes the core meaning of a generic to be a kind of vague quantifier (\emph{a la} ``many'' or ``a lot'') that derives its interpreted meaning as a function of richly structured prior knowledge about categories and properties. 
...
In the first section, we describe the model of generic meaning proposed by \citeA{Tessler2019psychrev} and develop a class of alternative ``fixed-threshold'' models that serve as tight comparisons for belief-updating mechanism proposed.
...
Here, we explored the hypothesis proposed by \citeA{Tessler2019psychrev} that generics update beliefs according to an uncertain threshold model. 
