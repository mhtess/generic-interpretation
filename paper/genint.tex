...
How exactly this learning occurs, however, is unclear: Generics exhibit so much flexibility in how they are interpretated that a quantitative theory of how generics update beliefs has not only been not developed or tested, but is often explicitly dismissed.
...
Furthermore, the fact that the same model of generic meaning can explain both the variability in generic interpretations (a listener task) and generic endorsements (a speaker task) as shown by Tessler \& Goodman (2019) suggests that generics are an effective medium favor faithful transmission of knowledge between interlocutors. 
...
In this paper, we often parameterize the prior as a mixture of two Beta distributions, which has the feature that prevalence levels close to 0\% are never exactly 0\% but instead may just be low numbers. Intuitively, this close-to-zero prevalence component of the distribution corresponds to an ``accidental cause'' of the feature, which does not preclude the possibility that the property appears in a few individuals in a category that should not normally have that property present (e.g., a dog that has three legs, an albino raven, ...).
...
We then perform a series of quantitative analyses aimed at predicting the prevalence implied by the generic (Part B) from the prior distribution of prevalence (Part A), assuming different models of belief-updating from a generic. 
...
\citeA{wu2017asking} also found that probabilistic information conveyed using perceptual features (e.g., spatial extent) led to better understanding of probabilities, as measured by performance on a probabilistic search task.  Thus, our use of a perceptually-grounded slider bar could support participants' estimations of probability.
...
We take advantage of the finding in Expt.~1a that the prevalence priors elicited by asking about alternative categories are best-modeled as a mixture of two Beta component distributions. 
We imagine one component governs the creatures that have some causal mechanism that stably gives rise to the property, while the other component is a null distribution, describing those creatures for which the feature is could only be generated by an extrinsic, transient cause. 
Our prior elicitation task is then a structured elicitation task, where ask about particular parameters assuming this model of the prevalence prior, and use Bayesian data analysis to reconstruct the prevalence priors implied by the responses to this structured elicitation task. 
...
categories as did in Expt.~1a) is difficult because many of the properties are fantastical
or obscure (e.g., pink teeth, silver legs, fungus-covered fur) which almost no familiar animals categories possess. 
Therefore,  we interrogate participants' latent beliefs about the property by assuming the mixture distribution validated in Expt.~1a and create an elicitation task to measure two aspects of the prior: (1) the relative contribution
of the null distribution (e.g., for how many categories is the
...
from Cimpian et al. (2010). \red{List of properties is shown in Table 1 of
the Appendix.}
...
\caption{\label{fig:cimpian-modelingResults-scatters}Posterior predictive model fits for eight models in terms of their fit for (A) item means and (B) full distribution of responses for each individual item. Full distribution plots are created by discretizing the distributions to bins of width 0.05 (20 bins per item). Models are (from left to right): (i) the mean of the prevalence prior, (ii - v) a threshold semantics fixed at 0.01 (\enquote{some}), 0.1, 0.3, and 0.5 (``most''), (vi) a threshold semantics with a threshold inferred from the data (``best threshold''), (vii) the ``reified kinds'' model that picks out the component of the prior distribution with a higher mean, and (viii) the uncertain threshold model. Fixed-threshold models are outfitted with a noise parameter to accommodate data points that are logically impossible given the fixed semantics. Error bars denote bootstrapped 95\% confidence intervals for the behavioral data and 95\% highest posterior density intervals for the model predictions.}
\end{figure}
...
\section{Experiment 3: Prior Manipulation}\label{experiment-2-prior-manipulation}}
...
%
The experimental data we present shows that generics can receive a spectrum of prevalence-based interpretations, including being interpreted as only applying to a minority of the category (\emph{weak generics}). 
...
These results stand in contrast to a common view in the psychological literature on generics that generics (in a generic sense) convey monolithic, strong meanings.\footnote{
	Gradience in property prevalence interpretations (e.g., how many wugs have seizures), which we examine in this paper, is distinct from gradience in property knowledge (e.g., whether or not an individual episode could be a gradient example of a seizure, such as an event which is a borderline case between a seizure and a non-seizure). Our results cannot speak one another or another as to whether or not property knowledge \emph{per se} is gradient, just that knowledge of prevalence is gradient. It has been shown, for example, that the response measure can elicit gradience from objectively binary properties \cite<e.g., the ``evenness'' of numbers;>{armstrong1983some}.\mht{improve this...}
}
Here, we trace the variability of generic interpretations back to prior beliefs, demonstrating how generics can convey strong implications, weak implications, and all shades in-between.
