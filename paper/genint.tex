...
\abstract{
Generic language (e.g., ``Birds have hollow bones'') conveys generalizations about categories and is believed to be a fundamental and ubiquitous mechanism of learning. 
...
Recently, Tessler and Goodman (2019) proposed a model for a meaning of generics as a vague quantifier, which operates over rich prior knowledge about properties; this model predicted human truth judgments for generics with a speaker decision rule. 
% Here, we test the adequacy of this model to account for the  belief-updating capacity of generic statements: How novel generics are
...
Language uniquely has the power to go beyond examples, conveying abstractions and encoding information that is itself a generalization.
...
A common intuition with generics is that they \enquote{{[}\ldots{}{]} appear to be commonly taken in a rather strong sense, as though the qualifier \emph{always} had implicitly crept into their interpretation} (p.~172; Abelson \& Kanouse, 1966). 
...
Intuitively, such strong interpretations could arise through pragmatic reasoning (Grice, 1975): Why would a speaker assert a generic claim unless the property (e.g., eating ants) were sufficiently prevalent in the category?

Viewing generics as fundamentally conveying high prevalence misses some key data points, however.
...
Generics like \enquote{Mosquitos carry malaria} only seem to have the quantificational force of an existential claim, meaning \emph{some mosquitos carry malaria} or \emph{mosquitos can carry malaria}.
...
Tessler and Goodman (2019) proposed that the core meaning of generics can be modeled as a vague quantifier, which operates over richly structured prior knowledge about categories and properties. 
Their work tested such a model only via \emph{truth judgments} or \emph{endorsements} (e.g., is \enquote{Mosquitos carry malaria} true or false?); the descriptive adequacy of such a model for belief-updating or interpretations has not been tested.
Under that model, a generic is functionally a quantifier (\emph{a la} ``some'', ``most'', ``all'') but one in which the listener does not know the threshold that the quantifier conveys (i.e., ``most'' means \emph{at least half}, whereas the generic means \emph{at least $\theta$}, where $\theta$ is unknown).
We adopt this model for the literal meaning of generics and incorporate it into the Rational Speech Act framework (Frank \& Goodman, 2012; Goodman \& Frank, 2016; Scontras, Tessler, \& Franke, 2018) to better understand how pragmatic reasoning enriches the literal interpretation of a generic.
...
In the first section, we review the model of generic meaning proposed by Tessler and Goodman (2019) and incorporate it into the Rational Speech Act modeling framework (Frank and Goodman, 2012; Goodman and Frank, 2016).
...
Tessler and Goodman (2019) propose that the literal contribution of a generic sentence is a function on a listener's subjective probability that a future instance of the category $k$ will have the property $f$; in the psychological literal on generics, such a probability is often called \emph{prevalence}, though the term \emph{projectibility} may be more appropriate (for consistency with that literature, we maintain the term prevalence). %some may call this ``projectibility''). 
...
We thus begin by considering a learner trying to figure out how strongly to expect the feature in the category: The learner considers the hypothesis that the prevalence of $f$ in $k$, $P(f(x){=}\texttt{true} | x\in k)$, is a particular value $r$. 

Tessler and Goodman (2019) borrow the machinery from formal semantics (Montague, 1973) to formalize the literal meaning of a generic as a threshold-function  (e.g., \(\mbox{ $[\![ gen ]\!]$} = r > \theta\)) whose threshold value is uncertain and is drawn from a uniform prior (\(\theta \sim \text{Uniform}(0, 1)\)). Thus, the generics model is like a model of a quantifier  (e.g., \(\mbox{ $[\![ some ]\!]$} = r > 0\), \(\mbox{ $[\![ most ]\!]$} = r > 0.5\), \(\mbox{ $[\![ all ]\!]$} = r = 1\)), but one in which the threshold is not specified a priori. 
...
\begin{align}
L_0(r, \theta \mid u) \propto {\delta_{\mbox{ $[\![ u ]\!]$}(r, \theta)} \cdot P(r) \cdot P(\theta)} \label{eq:L0}
\end{align}
...
Thus, given uniform prior beliefs about $r$ (i.e., all prevalence levels are equally likely \emph{a priori}), this model produces a posterior distribution that favors higher values of $r$, with probability proportional to their value (Figure~\ref{fig:tripartite}A, top panel). 
 ...
 The vague quantifier generics model produces inference about prevalence that are highly sensitive to prior beliefs about the property. 
 Prevalence prior distributions that display more structure (e.g., mutlimodal distributions) receive interpretations that are highly reflective of that underlying structure.
 This is a desirable feature because generics operate over rich intuitive theories about properties \red{(Prasada, Leslie, Gelman, Cimpian, Rhodes)}  and are believed to receive interpretations that are sensitive to those theories \cite{Cimpian2010}. 
 For instance, according to a veridical theory of biological properties like \emph{flying} (like in ``Ducks fly''), the listener is assumed to know that such a property manifests in many categories with 0\% prevalence (e.g., 0\% of dogs, rabbits, etc\ldots{} \emph{fly}), but is present in other categories in very high proportions (robins, falcons, pterodactyls, etc\ldots{}); hence the distribution would have at least two modes: one around 0\% and another near 100\% (Figure~\ref{fig:tripartite}A, second row).\footnote{
...
These judgments can be traced back to two components of a computational model: (i) the shared beliefs about the property in question represented by the prior distribution over prevalence $P(r)$ (Eq. \ref{eq:L0}) and (ii) endorsement as a speech act in which a sentence is judged as relatively better to achieve a speaker's goals than some set of alternative utterances. 
Formally, the view of truth judgments as an endorsement speech act can represented by a speaker reasoning about the listener model defined by Eq. \ref{eq:L0}.
The speaker reasons about whether the generic statement would be good at conveying the speaker's beliefs or whether an alternative would be better; the object of communication for the truth judgment task is the speaker's subjective prevalence (which is sometimes the objective frequency) and the space of alternative utterances only includes an informationless, silent alternative.\footnote{
