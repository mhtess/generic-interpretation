...
The precise articulation of how generic language updates beliefs, however, is beyond the reach of any extant theory of generics.
...
This earlier work tested such a model only via \emph{truth judgments} or \emph{endorsements} (e.g., is \enquote{Mosquitos carry malaria} true or false?); the descriptive adequacy of such a model for belief-updating or interpretations has not been tested.
...
One plausible source for deriving stronger interpretations is social, communicative reasoning (Grice, 1975): Why did the speaker bother to tell me this generic sentence (Figure~\ref{fig:cartoon})?
...
In this paper, we test a literal model of generic interpretation against a pragmatic model, which we formalize in the Rational Speech Act framework (Frank \& Goodman, 2012; Goodman \& Frank, 2016; Scontras, Tessler, \& Franke, 2018).
...
\caption{\label{fig:cartoon}Cartoon of the pragmatic generic interpretation model. The literal contribution of a generic (e.g., \enquote{Dorbs have yellow crests}) is equivalent to observing a minimal example: a single positive observation with only the predicated feature observed (i.e., observing an actual dorb with a yellow crest). 
...
We thus begin by considering a learner trying to figure out how strongly to expect the feature in the category: The learner considers the hypothesis that the prevalence of $f$ in $k$, $P(f(x){=}\texttt{true} | x\in k)$, is a particular value $r$. In the setting of observations, we denote the function that maps an observed item to all of its observable features as $F(x)$; we imagine the learner observes a particular $x$ in category $k$ that has the property of interest  among its features: $f(x)=\texttt{true}$. Bayes' rule then tells us:
$$
