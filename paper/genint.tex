...
How exactly this learning occurs, however, is unclear: Generics exhibit so much flexibility with respect to what makes them true or false that a quantitative theory of how generics update beliefs has not only been not developed or tested, but is often explicitly dismissed. % been developed and tested.
...
Here, we explore a hypothesis introduced by Tessler and Goodman (2019) that generics update beliefs like a vague quantifier: A category--property generic (\emph{Ks F}) means that the property F is expected to relatively widespread in the category K, where what counts as being relatively widespread depends upon prior beliefs about the property. 
Across two experiments, we both measure and manipulate prior beliefs. We find that prior beliefs are causally related to generic interpretations, as predicted, and that the variability exhibited by generics in endorsements (true/false judgments) manifests in a directly analogous manner in interpretations. %generics are best modeled as a vague quantifier.
...
The most basic, theory-neutral dimension of belief-updating about categories and properties is the prevalence of a feature in a category, or the probability that an instance $x$ of the category $k$ has feature $f$: $P(f(x){=}\texttt{true} \mid x\in k)$, or $P(f \mid k)$ for short-hand.
In this regard, a common intuition with generics is that they \enquote{{[}\ldots{}{]} appear to be commonly taken in a rather strong sense, as though the qualifier \emph{always} had implicitly crept into their interpretation} \cite<p.~172,>{Abelson1966}; in other words, $P(f \mid k)$ is very high or close to 1. 
Indeed, adults rate the prevalence of a feature (e.g., the percentage of bears that like to eat ants) implied by a generic (e.g., \emph{Bears like to eat ants}) as very high \cite<mean implied prevalence $\sim 85\%$;>{Gelman2002, Cimpian2010}.
...
Consistent with this intuition, \citeA{Cimpian2010} (Expt.~3) found lower implied prevalence from generics about features that could be construed as accidental (e.g., \emph{Crullets have fungus-covered claws}; see also \citeNP{Khemlani2009, Khemlani2012} for evidence of similarly weak inferences from generics such as \emph{Lions eat people}).
%Even more puzzling, generics can take on other interpretations --- distinct from \emph{all}, \emph{most}, or \emph{some} --- as in the case of \enquote{Ducks lay eggs}, where half of ducks (the females) lay eggs.
...
The fact that generic statements have such variable interpretations in terms of the property prevalence $P(f \mid k)$ has been taken by many in psychology to imply that there is no coherent relationship between the meaning of generics and the prevalence of the feature \cite{Leslie2008, Cimpian2010, Khemlani2012, Prasada2013}. 
Instead, theory-building has proceeded with verbal accounts, appealing to notions of kindhood and dangerousness, whose relation to quantitative data is often underspecified.
...
A systematic relationship between generics and prevalence could exist, however, and yet not be fixed across contexts: Generics, like many other aspects of language more generally, may exhibit a sensitivity to context that modulates, in a consistent predictable way, a fundamentally prevalence-based meaning (see \citeNP{kochari2020generics} for a related argument). 
...
\citeA{Tessler2019psychrev} proposed such a context-based, statistical account of generics. 
The model takes the core meaning of a generic to be a kind of vague quantifier (\emph{a la} \emph{many} or \emph{a lot}) that derives its interpreted meaning as a function of prior knowledge about categories and properties. 
Under this model, a generic acts like a quantifier (\emph{a la} \emph{some}, \emph{most}, \emph{all}) in the sense that its truth conditions are determined by a threshold function (e.g., \emph{some} means at least one,  \emph{most} means at least half); the generic is a special kind of quantifier, however, because the threshold for the generic is underspecified and must be inferred in context (i.e., the generic means: at least $\theta$, where $\theta$ is unknown).
...
Though initially developed  by \citeA{Tessler2019psychrev} to predict truth judgments or endorsements (e.g., is the sentence \emph{Mosquitos carry malaria} true or false?), the uncertain threshold model of generics also makes predictions concerning interpretations, or how beliefs should be updated, from generic language. These predictions have never been tested, however, and it is on interpretations that we focus on in this paper.
%It is these
...
In the first section, we describe the model of generic meaning proposed by \citeA{Tessler2019psychrev} and develop a class of alternative fixed-threshold models that serve as tight comparisons for the belief-updating mechanism proposed.
...
Then, in a preliminary study, we replicate and extend the findings of \citeA{Cimpian2010} (Expt.~3) that biological and accidental properties receive differential interpretations and find substantial variability in implied prevalence by-item; the items that exhibit the most variability, however, have a potential confound which leads us to design our main experiments.
In Experiment 1, we use a diverse stimulus set better able to elicit reliable variability in implied prevalence; we measure both prior beliefs and generic interpretations in order to evaluate and compare the quantitative models of generic interpretation and the alternative models. 
...
Finally, some properties may be expected to be present in only a minority of the category (e.g., because of a weak causal connection between the property and the kind such as \emph{carries malaria}; cf., \citeNP{Prasada2013}) which would follow a distribution  favoring low prevalence values (Figure \ref{fig:modelSimulations}B, bottom row). 
...
%\footnote{
%Some of the interpretation distributions shown in Figure \ref{fig:modelSimulations}B appear bimodal. In fact, they are all bimodal distributions to some extent given the parameterization of the prior; in many cases, however, the component of the distribution that favors very low prevalence levels has very low probability. 
%}

The formalization of prior beliefs in this model and the vague semantics has been used to explain human truth judgments (or, endorsements) of generics (e.g., that ``Robins lay eggs'' is intuitively true, while ``Robins are female'' is not) as well as other kinds of linguistic expressions that are thought to convey generalizations \cite<e.g., habituals;  ``Sally runs'' is less felicitous if Sally ran 3 times in the past year than if she ran 3 times in the past week;>{Tessler2019psychrev}.
...
\citeA{Cimpian2010} reported differences in the implied prevalence between biological properties (in particular, body parts with color modifiers e.g., \emph{yellow fur}) and accidental properties (e.g., \emph{fungus-covered claws}).
...
Experiment 1 is a larger scale version of \citeA{Cimpian2010}'s \emph{implied prevalence} task, with sufficiently dense measurements on the by-item level to reveal graded, quantitative variability among interpretations of generics of different properties. 
...
In order to motivate participants to reason about novel kinds, participants were told they were the resident zoologist of a team of scientists on a recently discovered island with many unknown animals, and their task was to provide their expert opinion on questions about these animals.
On each trial, participants were supplied with a bare plural sentence of the form \emph{Ks have F}, where K was a novel category label and F was a familiar property (e.g., \emph{Feps have yellow fur}).
...
It is not a measure of unconscious probabilistic expectations, which can often deviate from this kind of conscious probabilistic judgment \cite<e.g.,>{wu2017asking}.

\subsubsection{Materials}

The familiar properties were mostly taken from \citeA{Cimpian2010}, though were elaborated to create more classes of stimuli (the original stimuli only used color body parts and accidental properties). 
...
As in the replication of \citeA{Cimpian2010}, we observe a clear gradient in the implied prevalence ratings across our seventy-five items (Figure~\ref{fig:genint-empiricalData}).
...
This extreme gradience also stands in contrast to the results of \citeA{Cimpian2010}, which left open the possibility that were just a few special distinctions of properties that had unique implied prevalences (e.g., biological~vs.~accidental properties; see Figure \ref{fig:cimpian-modelingResults}). 
...
\begin{center}
  \begin{table}[h]
    ...
    \caption{Kolmogorov-Smirnoff test results testing for the dissimilarity between the priors (Expt.~2a) and the generic interpretation (posterior) response distributions (Expt.~2b). The only distributions to be not significantly different between the prior and posterior were the 50/100 and 75/100 distributions; the uncertain threshold model predicts only subtle shifts for these distributions.}
    \label{tab:ks}
  \end{table}
\end{center}
...
This uncertainty in the interpretation data is also a reason why the \emph{Reified kinds} model falls short; this model always selects the component of the prior distribution with higher prevalence. 
...
This work contributes a computational formalism to describe how beliefs are updated from generic language, which appears to be adequate within the domains considered; it represents a first step in understanding how generics serve to convey knowledge between people and generations.


%
The experimental data we present shows that generics can receive a spectrum of interpretations, including being interpreted as only applying to a minority of the category (\emph{weak generics}). 
...
When manipulating the prevalence prior distributions to suggest only prevalence levels of around 0\% or around 25\%, participants readily interpret the statement \emph{Ks know when earthquakes are about to happen} to mean about a quarter of them do (Expt. 2b).
...
These results stand in contrast to a common view in the psychological literature on generics that generics (in a generic sense) convey strong meanings.
Here, we trace the variability of generic interpretations back to prior beliefs, demonstrating how generics can convey strong implications, weak implications, and all shared in-between.

\citeA{Tessler2019psychrev} showed that the generic endorsement judgments (e.g., do you endorse the statement:  \emph{Mosquitos carry malaria}?) can be predicted by a model of a speaker deciding whether or not to produce the generic to a naive listener, where the naive listener updates its prior beliefs via an uncertain threshold function. 
We show here that the model of the naive listener---the \emph{Uncertain threshold model}---perfectly tracks with the generic interpretation data (Expt.~1b, 2b), in a manner that closely resembles how the predictions of the  speaker model of \citeA{Tessler2019psychrev} closely tracked with the generic endorsements.
This pair of facts---the speaker model of \citeA{Tessler2019psychrev} predicts generic endorsements, and the listener model (about which the speaker model reasons) predicts generic interpretations---points to a more general conclusion that our unified, model-based approach warrants: Generic endorsements and interpretations behave as one would expect if speakers and listeners are rational users of language. 
More broadly, speakers and listener use and interpret generics in a manner indicative of a mechanism for faithful transmission of knowledge. 
...
There are some important features of our paradigm that make our case study a kind of ``best case'' scenario for finding evidence for faithful knowledge transmission, and thus, potentially limit the generalizability of this conclusion. 
The first is that our stimuli are all generics about animals categories and properties, the statistics of which are relatively common knowledge.
...
The second feature of our design is that our studies are done with adults.
...
This conclusion is often arrived at by implicitly considering only fixed-threshold models of a prevalence-based semantics.
Our comparisons to models that assume fixed-thresholds support the narrow conclusion that a fixed-threshold semantics for generics is difficult to support given the data.
...
The meanings of gradable adjectives are often arrived by considering a comparison class of other objects of the same type \cite<e.g., an expensive hamburger is one that has a price that is high relative to other hamburgers;>{Kennedy2007} and the interpretation of vague adjectives can be modeled using an uncertain threshold in a way completely analogous to how we have modeled the meaning of generics \cite{Lassiter2015}.
...
Perhaps participants answer the prevalence prior questions (e.g., \emph{What percentage of dogs develop phobias?}) by first asking themselves internally a generic question (\emph{Do dogs develop phobias?}) and then responding with their personal, generic-interpretation prevalence if the answer to this question was \emph{yes} (and rating 0\% prevalence if the answer to the generic question was \emph{no}).
This procedure would be consistent with a view of generics as representationally primary (as posited by \citeNP{Prasada2000} in the form of \enquote{generic beliefs}) and in which prevalence estimates are derived by participants' generic knowledge. 
...
A more sophisticated version of this hypothesis is formalized by the \emph{Reified kinds} model, which explicitly reasons about the prior as a mixture of two distribution and selects the distribution with the higher mean prevalence value.
While the predictions of this model closely track the predictions of the \emph{Uncertain threshold} model for many generics, the \emph{Reified kinds} model over-predicts the prevalence ratings for generics that convey weak interpretations (e.g., \emph{Dorbs live in zoos.}). 
...
Instead, it is most parsimonious to conclude that from these experiments that generic belief update prior beliefs about prevalence in a graded fashion, modeled as an uncertain threshold rule.
