...
\begin{figure}[ht]
  \begin{center}
    \begin{tabular}{cc}
\begin{tikzpicture}
  \node[obs]          (d-c)   {$d^f_{gen}$}; %
  \node[obs, left=3.5 of d-c, yshift=2cm]          (x)   {$d^f_{prior}$}; %
  \node[latent, above=1cm of x, xshift=-0.5cm] (mx) {$\phi^f$} ; %
  \node[latent, above=2cm of x, xshift=0.75cm]  (sx) {$\gamma^f_i$} ; %
  \node[latent, above=2cm of x, xshift=1.75cm]  (dx) {$\delta^f_i$} ; %
%  \factor[above=of x] {x-f} {left:$\text{mixture of Betas}$} {mx, sx,dx} {x} ; %
  ...
  \node[det, above=0.75cm of d-c] (L1) {$L_1(x \mid u)$} ; % 
...
\node[] at (4.5,4) {$\phi_f \sim \text{Uniform}(0, 1)$};
%\node[] at (4,5.5) {$\beta^0_1, \beta^1_1 \sim \text{Uniform}(-3, 3)$};
\node[] at (4.5,3.25) {$\gamma^f_i \sim \text{Uniform}(0, 1)$};
\node[] at (4.5,2.5) {$\delta^f_i \sim \text{Exponential}(1)$};
