...
\abstract{
Generic language (e.g., ``Birds have hollow bones'') conveys generalizations about categories and is a fundamental and ubiquitous mechanism of learning. 
...
Recently, Tessler and Goodman (2019) proposed a model for a meaning of generics as a vague quantifier, which operates over rich prior knowledge about properties; this model predicted human truth judgments for generics with high quantitative accuracy. 
%Here, we test the adequacy of this model to account for the belief-updating capacity of generic statements: How novel generics are interpreted by listeners.
...
These models also predict strong dependence of interpretation on prior beliefs about the prevalence of features. Across two experiments, we both measure and manipulate prior beliefs. We find that prior beliefs are causally related to generic interpretations, as predicted, and that generics are best modeled as a vague quantifier understood with a communicative intent.
...
Language uniquely has the power to describe that which goes beyond what can be experienced, conveying abstractions and generalizations.
Generalizations about categories are often conveyed using \emph{generic language} \cite<e.g., ``Birds have hollow bones''>{Carlson1977}, an important case of abstract knowledge transmitted through language \cite{Gelman2009}.
...
A common intuition with generics is that they \enquote{{[}\ldots{}{]} appear to be commonly taken in a rather strong sense, as though the qualifier \emph{always} had implicitly crept into their interpretation} \cite<p.~172, >{Abelson1966}. 
Indeed, adults learning from generics about both familiar and novel animals (e.g., \enquote{Bears like to eat ants}; \enquote{Lorches have purple feathers}) rate the \emph{implied prevalence} of the feature (e.g., the percentage of bears that like to eat ants) as very high \cite<mean implied prevalence $\sim 85\%$;>{Gelman2002, Cimpian2010}.
...
Viewing generics as fundamentally conveying high prevalence misses some key observations with respect to generics, however.
...
Sentences like \enquote{Mosquitos carry malaria} only seem to have the quantificational force of an existential claim, meaning \emph{some mosquitos carry malaria} or \emph{mosquitos can carry malaria}.
Consistent with this intuition, \citeA{Cimpian2010} (Expt.~3) found lower implied prevalence from generics about features that could be construed as accidental (e.g., ``Crullets have fungus-covered claws''; see also \citeNP{Khemlani2009, Khemlani2012} for evidence of weak inferences from generics such as ``Lions eat people'').
Even more puzzling, generics can mean neither \emph{some} nor \emph{most} as in the case of \enquote{Ducks lay eggs}, where half of ducks (the females) lay eggs.
...
\citeA{Tessler2019psychrev} proposed that the core meaning of generics is a kind of vague quantifier that operates over subjective, predictive probabilities that result from richly structured prior knowledge about categories and properties. 
...
In the original model of \citeA{Tessler2019psychrev}, the threshold is resolved by a listener reasoning about the prior probabilities of different levels of prevalence for a feature; for example, \emph{carrying malaria} is a property whose prevalence in a category is expected to be low because even when a member of a certain category is \emph{carrying malaria}, it is relatively unlikely that most or all other members of the category are also \emph{carrying malaria}.
...
Similarly, inferences that members of a category have a property (e.g., \enquote{Leo is a lion. Thus, Leo eats people.}) can be quite weak, though still the extant empirical evidence suggests participants think this inference is more likely than not \cite{Khemlani2012}.\footnote{In this study of \citeA{Khemlani2012}, the measurement was actually a confidence rating on -3 to +3 scale. All reported mean confidence ratings were greater than the midpoint of the scale.}
...
In the first section, we review the model of generic meaning proposed by \citeA{Tessler2019psychrev} and incorporate it into the Rational Speech Act modeling framework \cite{Frank2012, Goodman2016}.
...
%The speaker's decision is relative to a set of alternative utterances; since Tessler and Goodman (2019) were interested in modeling sentence endorsements, the alternative set consisted only of producing the generic and staying silent.
%The act of silence is an information-less utterance that produces a posterior distribution in the literal listener that is identical to the prior and incorporating such an utterance into a pragmatic model is a minimal mechanism go encode speaker intentionality into utterance production: Why did the speaker choose to say the utterance they said?
%In summary, in addition to the cost of the utterance, the speaker's production decision reflects the prevalence level the speaker intends to convey, $r$ (i.e., their belief about how many of the category have the property), under the literal generic interpretation distribution~vs.~the prior distribution The modeled truth judgments as a speaker decision of whether or not to say the generic sentence to the listener defined by Equation \ref{eq:L0}.
%Following a typical Rational Speech Act model (Frank and Goodman, 2012; Goodman and Frank, 2016), the speaker is a soft-max rational agent whose utility function is given by the information conveyed to the listener by the utterance $u$ -- $\ln L_0(r \mid u)$ -- minus the cost of the utterance $C(u)$ (e.g., taking into account that certain utterances will be costlier to produce because they are longer):
%(Figure~\ref{fig:tripartite}A). 
...
%\red{END EDITS}
%In the setting of observations, we denote the function that maps an observed item to all of its observable features as $F(x)$; we imagine the learner observes a particular $x$ in category $k$ that has the property of interest  among its features: $f(x)=\texttt{true}$. Bayes' rule then tells us:
%$$
%P(r \mid F(x); x \in k) \propto P( F(x) \mid r; x \in k) \cdot P(r)
%$$	
%In the setting of a minimal example, the learner observes that $x$ has property $f$, but has not observed any of its other properties.
...
%\noindent Equation \ref{eq:L0} is derived from the principle that belief updating from examples is cognitively available as a means of generalization, and so generics may make use of it (cf., Leslie, 2008).
%We show in Appendix A that this model is mathematically equivalent to the model in Tessler and Goodman (2019), wherein generics update beliefs according to an uncertain threshold function like a vague quantifier. 
%
...
%%with the overall preference for higher prevalence levels \(r\). %, which results from observing a positive example.
...
%	We assume that people have graded degrees of belief about the prevalence and hence the mode at 0\% is not a Delta distribution at exactly 0\%. That is, the model assumes there are categories in which almost no members have the property (e.g., a dog flying), but that the existence of such an individual is not entirely impossible.}
%	%Bayesians in general do not like to assign 0 probabilities to outcomes, which is what a mode at 0\% would entail for this cognitive model. 
%	%Instead, the model assigns some low but non-zero probability that an instance of a category having a property which is not in general true of the category  (e.g., a dog flying), because it is not impossible to imagine some intuitive chain of causal events that could give rise to the property (e.g., a genetic mutation or a cross-species cloning experiment). }
...
%Belief updating based on an underspecified threshold produces posterior distributions that are radically different depending on the prevalence prior distribution  (Figure \ref{fig:tripartite}A, solid lines).
...
Experiment 1 replicates and extends this result; this study serves to validate that the paradigm can elicit fine-grained quantitative variability, which we will build on to test the computational models.
Experiment 2 is a larger scale version of \citeA{Cimpian2010}'s \emph{implied prevalence} task, with sufficiently dense measurements on the by-item level to reveal graded, quantitative variability among interpretations of generics of different properties. 
...
On each trial, participants were supplied with a bare plural sentence of the form ``Ks have F'', where K was a novel category label and F was a familiar property (e.g., \enquote{Feps have yellow fur}).
...
This experiment is designed to measure the prevalence implied by a generic about a novel kind, using a diverse set of properties.
We first elicit participants' prior beliefs about the likely prevalence levels expected for these properties (Expt. 2a).
We then use the elicited prevalence priors to make model predictions about the prevalence implied by a generic (Expt. 2b).
...
\subsection{Experiment 2b: Generic interpretation}\label{experiment-1b-generic-interpretation}}
...
This extreme gradience stands in contrast to the results of \citeA{Cimpian2010}, which left open the possibility that were just a few special distinctions of properties that had unique implied prevalences (e.g., biological~vs.~accidental properties; see Figure \ref{fig:cimpian-modelingResults}). 
...
Our model-based analyses ask how well the various belief-updating models accommodate the human generic interpretation data and which model provides the most parsimonious view of the data.
We estimate all model parameters using Bayesian methods in order to determine the most likely values of the latent parameters given our hypotheses about how both the prior elicitation and generic interpretation data were generated.\footnote{Appendix C in \citeA{Tessler2019psychrev} provides a careful analysis of the behavior of this joint Bayesian inference strategy for jointly modeling the prevalence prior parameters together with other parameters of the model governing generic interpretation. They find imperceptibly small differences between the parameters inferred from the prevalence prior data in isolation and those inferred by jointly modeling the prior elicitation together and the generic interpretation data, suggesting that in order to provide a stronger quantitative fit to the generic interpretation data, the prevalence priors are not significantly distorted by this method.}
...
For each candidate model of generic interpretation (literal, pragmatic, \enquote{some}, \enquote{most}), we build a joint Bayesian data-analytic model to simultaneously predict the prevalence prior data (Expt. 2a) and the generic interpretation data (Expt. 2b), similar to the methods of Tessler and Goodman (2019) (Figure \ref{fig:bayesnet}).
We model the prevalence prior data as coming from a mixture of two Beta distributions (as determined in Expt.~2a) and we model the generic interpretation data as being generated from the candidate computational model of generic interpretation.
%For data analysis, we assume that the prevalence priors take the form of a three-component mixture of Beta distributions\footnote{We use Beta distributions to match the form of the response variable: a number between 0 - 1.}, whose parameter uncertainty is strongly constrained by data from our prior elicitation task.
Each prevalence prior has five parameters governing its shape: a mean \(\gamma\) and variance parameter \(\xi\) for each of the two Beta components and a mixture parameter $\phi$ that describe the relative weighting among the two components.
We put uninformative priors over these parameters \(\gamma \sim \text{Uniform}(0, 1)\), \(\xi \sim \text{Uniform}(0, 100)\), \(\phi \sim \text{Uniform}(0,1)\).\footnote{Another reasonable, less structured model of the prevalence prior data would be a Dirichlet over discretized bins along the prevalence scale. We choose the mixture-of-Betas formulation because it assumes some correlation between neighboring values along the scale and has fewer parameters. }
...
In addition, the model of \enquote{most} semantics has an additional noise parameter, which adds a probability \(\omega\) of a participant responding randomly; we put a uniform prior over the value of this parameter: \(\omega \sim \text{Uniform}(0, 1)\).
...
Table 1 shows the mean squared errors and proportion of explained variance for predictions based on the prevalence prior alone, a literal \enquote{some} model, a literal \enquote{most} model (which includes an additional fit noise parameter), a literal uncertain-threshold generics model, and the pragmatic uncertain-threshold generics model.
Figure~\ref{fig:genint-modelingResults} reveals that both the prior only model and the belief-updating model based on the quantifier \enquote{some} (i.e., a fixed threshold at the lowest possible value) consistently \emph{underpredict} the implied prevalence, consistent with the intuition that assigning the semantics of \enquote{some} for generics is too weak.
