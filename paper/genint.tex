...
Given that human generalization from observations can be well described using the language of probability (Shepard, 1987) and Bayesian belief-updating with (often structured) prior knowledge (Tenenbaum et al., 2011),  Tessler and Goodman (2019) propose that the meaning of generics---which convey generalizations---can also be formalized using the probability calculus.
...
$P(\theta)$ is the prior distribution over semantic thresholds for the generic sentence (assumed to be uniform) and $P(r)$ is the learner's prior beliefs about the prevalence (plausibly varies by content and context).
...
Tessler and Goodman (2019) propose that belief-updating from a generic is equivalent to stating that the bias of the coin is greater than $\theta$, where $\theta$ is sampled from a uniform prior probability distribution.

If $\theta$ is low, then many prevalence levels $r$ will be consistent with the threshold semantics.
If $\theta$ is high, only the highest prevalence levels will be consistent with the threshold. 
