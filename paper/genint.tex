...
The pragmatic generic interpretation models interprets a generic as conveying a communicatively relevant, minimal example to the listener. 
...
Thus, using generics for conveying generalizations is more efficient because the speaker isolates a feature for generalization. 
Generics are also more useful than observations because speakers can describe properties that listeners could not in principle observe (Harris \& Koenig, 2006). 
...
The view of generics as communicatively relevant examples has previously been suggested by Csibra \& Shamsudheen (2015), who argue that individual objects are interpreted by infants as symbolic representations of the object kind, and thus that it is possible to interpret a pedagogically presented exemplar from a category as a \enquote{nonverbal generic}.
The pedagogical example acts as an indexical to the category.
In fact, the connection between generics and indexicality has made been by Sterken (2015) as well, arguing that the context-sensitivity exhibited by indexicals is the exact kind we need to model generics.
...
The minimal example is more concrete, however. 
Consider the case of trying to infer the weight of a biased coin: The quantifier ``some'' tells you that both sides are not tails, whereas a minimal example requires actually flipping the coin and seeing it land on heads. 
The minimal example entails existential quantification, but Bayesian reasoning can update based on an example in a stronger way.
Without strong prior knowledge about the weight of a coin, it is more likely than not that it has a heads bias after observing just a single heads outcome; in fact, the most likely state of affairs to produce a single heads outcome (given uniform uncertainty about the weight of a coin) is that both sides of the coin are heads.
