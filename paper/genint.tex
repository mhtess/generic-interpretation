...
\begin{tikzpicture}
  ...
  \node[latent, above=2cm of x, xshift=1.75cm]  (dx) {$\delta^f_j$} ; %
%  \factor[above=of x] {x-f} {left:$\text{mixture of Betas}$} {mx, sx,dx} {x} ; %
  \edge[above=of x]{mx, sx,dx} {x} ; %
      \node[latent, right=2cm of x]  (xf)   {$x^{f}$}; 
  \edge[above=of xf]  {mx, sx,dx} {xf} ; %

  \node[det, above=0.6cm of d-c] (L1) {$L_1(x^f \mid u)$} ; % 
...
$\phi_f \sim \text{Uniform}(0, 1)$ \\
 $\gamma^f_j \sim \text{Uniform}(0, 1), \delta^f_j \sim \text{Exponential}(1)$ \\
  $d^{prior}_{f, i'} \sim \phi^f \cdot \text{Beta}(\gamma^f_1, \delta^f_1) + (1-\phi^f) \cdot \text{Beta}(\gamma^f_2, \delta^f_2)$ \\
    $x^f \sim \phi^f \cdot \text{Beta}(\gamma^f_1, \delta^f_1) + (1-\phi^f) \cdot \text{Beta}(\gamma^f_2, \delta^f_2)$ \\

 \\

$ d^{gen}_{f, i} \sim L(x^f \mid u  = \text{generic}) = \int_{\theta} L(x^f, \theta \mid u) \diff \theta$
};
...
$L_1(x^f, \theta \mid u) \propto S_1(u \mid x^f, \theta) \cdot P(x^f) \cdot P(\theta) $\\ 
$S_1(u \mid x^f, \theta)\propto \exp{( \alpha \cdot [ \ln L_0(x \mid u, \theta) - c(u)])} $\\
$L_0(x^f \mid u, \theta) \propto  \delta_{\denote{u}(x^f, \theta)} \cdot P(x^f)$
};
...
For each candidate model of generic interpretation (literal, pragmatic, \enquote{some}, \enquote{most}), we build a joint Bayesian data-analytic model to simultaneously predict the prevalence prior data (Expt. 2a) and the generic interpretation data (Expt. 2b), similar to the methods of Tessler and Goodman (2019).
