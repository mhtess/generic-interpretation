...
The precise articulation of how generic language updates beliefs is, however, beyond the reach of any extant theory of generics.
Here, we formalize a simple hypothesis about the meaning of generics:  Generics convey a single, minimal example to the listener, borrowing the machinery of learning from observations. 
...
We know that belief updating from examples is a very basic human capability, shared with other animals. 
It is thus a reasonable starting point to imagine that language has coopted this belief updating mechanism: that the meaning of a generic is equivalent ``imagine you have observed an example.''
That is, hearing \enquote{Mosquitos carry malaria} is interpreted as the knowledge that one mosquito, in one particular circumstance, was once observed carrying malaria; belief updating from this one ``observation'' would effect our beliefs about other mosquitos, given that properties tends to be consistent within kinds.
...
It turns out, that construing generic interpretation as Bayesian belief-updating from a single minimal example is mathematically identical to a recent model by Tessler and Goodman (2019) that treats a generic as a vague quantifier.
...
Our formal modeling will suggest that such very weak interpretations can occur, when prior knowledge indicates that the property in question is very rare across categories.
...
We thus begin by considering the hypothesis that the prevalence of $f$ in $k$, $P(f(x){=}\texttt{true} | x\in k)$, is a particular value $r$. In the setting of observations, we imagine that we have observed a particular $x_\texttt{obs}$ in category $k$ that has the property of interest, $f(x_\texttt{obs})=\texttt{true}$. Bayes' rule then tells us:
$$
P(r | x_\texttt{obs}, x_\texttt{obs} \in k) \propto P( x_\texttt{obs} | x_\texttt{obs}\in k, r)P(r)
$$
In the setting of a minimal example, we have observed that $x_\texttt{obs}$ has property $f$, but we have not observed any of its other properties.
If we assume that properties are independent given the category, then unobserved properties can be marginalized away. Writing the object now as $x$, to remind us that we haven't fully observed it, we have:
\begin{align}
P(r | x \in k, f(x){=}\texttt{true}) &\propto P(f(x){=}\texttt{true} | x \in k, r) P(r) \nonumber \\
&\propto r\cdot P(r), \nonumber
\end{align}
where we have used the fact that the hypothesis, $r$, \emph{is} the likelihood in this setting.
Finally, adopting this minimal-example belief-update as the effect of hearing the generic utterance yields:
\begin{equation}
