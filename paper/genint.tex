...
Tessler and Goodman (2019) used this semantic hypothesis to explain human truth judgments (or endorsements) of generics (e.g., that ``Robins lay eggs'' is intuitively true, while ``Robins are female'' is not) as well as other kinds of linguistic expressions that are thought to convey generalizations (e.g., habituals;  ``Sally runs'' is less felicitous if Sally ran 3 times in the past year than if she ran 3 times in the past week).
They modeled truth judgments as a speaker decision of whether or not to say the generic sentence to the listener defined by Equation \ref{eq:L0}.
