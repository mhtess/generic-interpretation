...
We thus begin by considering a learner trying to figure out how strongly to expect the feature in the category: The learner considers the hypothesis that the prevalence of $f$ in $k$, $P(f(x){=}\texttt{true} | x\in k)$, is a particular value $r$. In the setting of observations, let us write $F(x)$ for all of the observable features of an observed item $x$; we imagine the learner observes a particular $x$ in category $k$ that has the property of interest, $f(x)=\texttt{true}$ among its features. Bayes' rule then tells us:
$$
