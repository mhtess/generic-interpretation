...
ggsave("../figs/literal_prior_silent.pdf", width = 4, height = 5)
```
...
wp.rs.l1.filtered %>% 
  ...
  scale_linetype_manual(values = c(1, 2))+
  scale_x_continuous(limits = c(0, 1), breaks = c(0, 1))+
  scale_y_continuous(limits = c(0, 1.7), breaks = c(0, 1.5))+
  guides(linetype = guide_legend(reverse = T, title = element_blank()))+
  theme(strip.text = element_blank())+
  coord_fixed(ratio = 1/2)#+
  #guides(linetype = F)

ggsave("../figs/literal_prior_generic.pdf", width = 6, height = 3.5)
```


```{r}
wp.rs.l1.filtered %>% 
  filter(modelName %in% c("pragmatic_unlifted", "prior"),
         cost == 10,
         priorName == "Xs Y\n[uniform]") %>%
  mutate(modelName = factor(modelName, levels = c( "pragmatic_unlifted", "prior"),
                            labels = c("Pragmatic generic interpretation",
                                       'Prior / "silent" interpretation'))) %>%
  rename(prob = prediction) %>%
  get_samples(., 750) %>%
  ggplot(., aes( x = state, linetype = modelName))+
...
ggsave("../figs/pragmatic_generic.pdf", width = 4.5, height = 3.5)
```


# L0 model with Joint Posterior

```{r}

inputData <- list(prior = list(params = list(
    c1 = data.frame(a = 1, b = 1), 
    c2 = data.frame(a = 1, b = 200)
  ), mix = 1))

rs.uncertain.threshold.model <- webppl(
  program_code = paste(
    rsaHelpers, 
    statePrior, 
    uncertain.threshold.model, 
...
ggplot(rs.uncertain.threshold.model, aes( x = state, y = theta))+
  stat_density_2d(aes(fill = prob), geom = 'polygon')
```

```{r literal exact discrete}
priorShapes <- list(
  uniform =  list(params = list(
    c1 = data.frame(a = 1, b = 1), 
    c2 = data.frame(a = 1, b = 200)
...
discrete.posteriors <- data.frame()
  
for (p in priorNames){
  priorProbs = priorShapes[[p]]
  
  discrete.posteriors.i <- expand.grid(
    state = seq(0, 1, 0.01),
    theta  = seq(0, 1, 0.01)
  ) %>% mutate(
    state_prior_prob = priorProbs$mix * 
      dbeta(shape1 = priorProbs$params$c1[1, "a"],
            shape2 = priorProbs$params$c1[1, "b"],
            x = state) + 
      (1 - priorProbs$mix) *
      dbeta(shape1 = priorProbs$params$c2[1, "a"],
            shape2 = priorProbs$params$c2[1, "b"],
            x = state),
    priorName = p,
    unnorm_post_density =  ifelse(state > theta, state_prior_prob, 0)
  )
  
  discrete.posteriors <- bind_rows(
    discrete.posteriors,
    discrete.posteriors.i
  )
  print(p)
}

  #rowwise() %>%
  # mutate(unnorm_prob_density = ifelse(state > theta, 1, 0)) %>%
  # #ungroup() %>%
  # mutate(prob_mass = unnorm_prob_density / sum(unnorm_prob_density))


plot_func <- function(df, name) {
  highest_post_densities <- sort(df %>% pull(unnorm_post_density), decreasing = T )
  single_point_outlier <- highest_post_densities[1] > 2*highest_post_densities[2]
  
  df %>% 
    rowwise() %>%
    mutate(unnorm_post_density = ifelse(single_point_outlier,
                                        ifelse(unnorm_post_density == highest_post_densities[1], highest_post_densities[2], unnorm_post_density), unnorm_post_density)) %>%
  ggplot(., aes( x = state, y = theta, fill = unnorm_post_density))+
    geom_tile()+
    labs(x = "r (prevalence)",
         y = expression(paste(theta, " (threshold)", sep ="")),
         title = name)+
    scale_x_continuous(breaks = c(0, 1))+
    scale_y_continuous(breaks = c(0, 1))+
    scale_fill_continuous(name = "unnormalized posterior density",
                          breaks = c(
                            last(highest_post_densities), 
                            first(highest_post_densities)
                            )
                          )+
...
nested_tmp <- discrete.posteriors %>% 
  group_by(priorName) %>% 
  nest() %>% 
  mutate(plots = map2(data, priorName, plot_func))

gridExtra::grid.arrange(grobs = nested_tmp$plots, nrow = 1)
