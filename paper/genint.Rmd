...
This property was chosen because the prevalence prior elicited in Expt. 2 suggested that participants had a lot of uncertainty as to what prevalence levels to expect for this property.
...
Information about the animal appeared in an *evidence statement* (e.g., \red{"You and your partner find that 98\% of Animal C *know when earthquakes are about to happen*."}) and were displayed in a table showing the corresponding numerical data \red{(e.g., "number with property"; Figure)}. 
Participants proceed by clicking a continue button to reveal information about the next animals.
After participants viewed the data for ten categories, they are told to review the information about the animals before continuing (\red{Figure}).

After the data table, participants were provided the prompt for either the *prior manipulation* condition or the *generic interpretation* condition.
...
Participants are then asked: "What percentage of Ks do you think know when earthquakes are about to happen?" to which they could respond with a rating on a slider bar ranging from 0%-100%. 
In the *prior manipulation* condition, participants were told they were going to study five more animals and asked to predict how many of each would have the property (\red{Figure}).
Participants were given five slider bars ranging from 0 - 100, and asked to predict the prevalence for the next five categories (Animals K - P).
...
\red{The prevalences shown following one of N distributions}.
...
prior.manip.toprow <- plot_grid(prior.manip.exp.1, 
                             prior.manip.exp.2, labels = c("A", "B", "C"), nrow = 1, align = 'vh')
...
plot_grid(
  ...
  plot_grid(add_sub(prior.manip.exp.3, "Manipulation check"), 
            add_sub(prior.manip.exp.4,"Generic interpretation"), labels = c("D", "E"), nrow = 1),
  nrow = 2)
```
