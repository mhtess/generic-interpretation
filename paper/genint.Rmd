...
As a first attempt to understand the implied prevalence data in a quantitative way, we compare the means of the inferred prevalence priors to the empirical means of the implied prevalence data (Figure\ \@ref(fig:genint-modelingResults) left-most facet).
...
Though the model explains a lot of the variance $r^2(`r filter(md.impprev.stats, semantics == "some", parameter == "posterior", src == "literal")[["n"]]`) = `r filter(md.impprev.stats, semantics == "some", parameter == "posterior", src == "literal")[["r2"]]`$, it does so with a relatively high mean squared error $MSE = `r format(filter(md.impprev.stats, semantics == "some", parameter == "posterior", src == "literal")[["mse"]], digits = 2)`$.
...
The literal generic interpretation model updates its beliefs as if the listener had observed a single, positive example of the category with the property, and the pragmatic generic interpretation model sees the single observation as one that was intentionally transmitted by the speaker.
Unlike each of the alternative models, the literal generic model can accomodate the highly variable generic interpretation data with a high degree of quantitative accuracy $r^2(`r filter(md.impprev.stats, semantics == "uncertain", parameter == "posterior", src == "literal")[["n"]]`) = `r filter(md.impprev.stats, semantics == "uncertain", parameter == "posterior", src == "literal")[["r2"]]`$, $MSE = `r format(filter(md.impprev.stats, semantics == "uncertain", parameter == "posterior", src == "literal")[["mse"]], digits = 2)`$.
The pragmatic generic interpretation model can provide an even stronger quantitative fit to the empirical data $r^2(`r filter(md.impprev.stats, semantics == "uncertain", parameter == "posterior", src == "pragmatic_unlifted")[["n"]]`) = `r filter(md.impprev.stats, semantics == "uncertain", parameter == "posterior", src == "pragmatic_unlifted")[["r2"]]`$, $MSE = `r format(filter(md.impprev.stats, semantics == "uncertain", parameter == "posterior", src == "pragmatic_unlifted")[["mse"]], digits = 2)`$.
...
Our second data analytic strategy is a formal model comparison to arbitrate between the literal and the pragmatic generic interpretation models. 
Comparing these models using a fully Bayesian strategy (analogous to what is performed in the model criticism section) is computationally challenging. 
Therefore, we compute the likelihood of the data under each model by fixing the prevalence priors to be the raw empirical distributions; fixing the prevalence priors reduces the overall flexibility of the models, but it does so in a way that penalizes both models to an equal extent.
To deal with the additional flexibility of the pragmatic model afforded by its two additional free parameters, we used an Annealed Importance Sampling algorithm implemented in the probabilistic programming language WebPPL to calculate the marginal likelihood of the data under the pragmatic model. 
