...
We ran the model using 3 chains of an incrementalized version of MCMC for 750,000 iterations, the first 250,000 of which were discarded for burn-in. 
...
The second is analagous to "most", with a threshold at 50% prevalence. 
...
We infer this probability for the "most" model from the data.
...
With those criteria in mind, we preregistered the following contrast predictions (\{\}s indicate no predicted difference): 0/25 $<$ \{25/50, 0/50\} $<$ \{25/75, *uniform*\} $<$ 25/75 $<$ \{25/100, 50/100\}  $<$ \{75/100, 0/100\}.
...
The important result is that even little bit of noise (between 1-5% of responses), the uncertain threshold model is quantitatively the better model.
