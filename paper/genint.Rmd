...
We explore a null hypothesis that understanding a generic statement is equivalent to observing a single positive instance of a category with a property and updating one's beliefs according to Bayes rule. 
We show that this model is mathematically equivalant to a listener model proposed @TesslerLangGenUnderReview, wherein generics operated as a kind of vague quantifier. 
The @TesslerLangGenUnderReview model was used not used to predict interpretations but rather endorsements (e.g., is "Mosquitos carry malaria" true or false?), modeled as speaker deciding whether or not to produce a generic statement to a listener.
The adequacy for the belief-updating component model of the model (the listener) has not yet been tested.
We compare the single positive observation model from a bonafide communicative model wherein generics are understood with a pedagogical intent. 
This model connects to a recent proposal by @Csibra2015 that generics can be understood nonverbally but with a communicative intent. 
...
We propose a the null hypothesis that generics convey the same information-content as a single, positive observation interpreting with respect to structured background konwledge. 
Literally, if "Mosquitos carry malaria" means something like *some mosquitos, under some circumstances, carry malaria* then the knowledge conveyed could be similar to that learning (i) one mosquito, in one particular circumstance, was once observed carrying malaria and (ii) given that properties covary with kinds in systematic way, if one mosquito once had this property, then it is reasonable to think there is some larger (but unknown) subset of mosquitos that in some larger (but unknown) class of situations, also carry malaria.

Formally, if a listener integrates their prior knowledge about the underlying probability or prevalence of the property $r$ with a single positive observation according to Bayes' rule, the model is:
...
The latter is simply the probability of a coin with weight $r$ to land on hands one out of one times, which is simply $r$.
Belief-updating based on a single positive observation is mathematically equivalent to a belief-updating based on a threshold-function (e.g., $\denote{gen} = x > \theta$), whose threshold value is uncertain and is drawn from a uniform prior ($\theta \sim \text{Uniform}(0, 1)$; see Appendix for derivation): 

\begin{eqnarray}
L(r, \theta \mid u) &\propto& {\delta_{\denote{u}(r, \theta)} \cdot P(r) \cdot P(\theta)} \label{eq:L0a}
\end{eqnarray}

Here, the truth-functional meaning is represented by the Kronecker delta function  $\delta_{\denote{u}(r, \theta)}$ that returns $1$ when the utterance is true (i.e., when $r > \theta$) and $0$ otherwise.
...
The uncertain threshold form of the model is that presented by @TesslerLangGenUnderReview and uses the  truth-functional tools of formal semantics [@Montague1973].
Such a literal meaning also relates to the literal semantics of quantifiers, which can also be described by threshold functions (e.g., $\denote{some} = \{r > 0\}$, $\denote{most} = \{r > 0.5\}$, $\denote{all} = \{r = 1\}$).
