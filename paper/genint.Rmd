...
Given a particular value of $\theta$, the semantics rules out all prevalences $p$ below $\theta$.
...
Higher prevalence levels are consistent with more thresholds.
Thus, the generic interpretation model, which has uncertainty over the threshold and marginalizes over different possible thresholds, returns a posterior that favors higher prevalence levels (Figure\ \@ref(fig:modelSimulations)B; top row, right-most column).
This behavior is consistent with the intuition that, without strong background knowledge, generics imply that most or all of the category have the feature.

Context-sensitive interpretations come from different prior distributions over prevalence $P(p)$, which can be thought of as levels of prevalence given different categories within a comparison class.
...
The prevalence prior for a purely abstract property may be given by a uniform distribution, while prevalence priors for familiar properties may display interesting structure.
...
Generic interpretation manifests as similar to different quantified statements depending on the context (Figure\ \@ref(fig:modelSimulations)B). 
For the biological property prior, "Xs fly" is interpreted most similarly to "Most Xs fly" and not "Some Xs fly".
For the accidental property prior, "Xs carry malaria" is most similar to "Some Xs carry malaria" and not "Most Xs carry malaria". 
The generic interpretation model balances the prior probability of different prevalence levels with the overall preference for higher prevalence levels (as described above), producing subtle context-sensitive interpretations.
...
```{r cimpian-prevPrior, fig.width = 9, fig.asp = 0.4, fig.cap="Prevalence priors."}
...
```{r cimpian-modelingResults, fig.width = 10, fig.asp = 0.57, fig.cap="Model fits."}
...
```{r genInt-prevPrior, fig.width = 11, fig.asp = 0.36, fig.cap="Prevalence priors."}
