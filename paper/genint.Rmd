...
Using language, a teacher can go beyond examples, conveying information that is itself a generalization.
...
In this paper, we show how this formalism is mathematically identical to a recent model by @Tessler2019psychrev that generics are a kind of vague quantifier. 
...
One plausible source for why stronger interpretations could be derived is from social reasoning: Why did the speaker bother to tell me this generic sentence?
...
The idea that generics convey a kind communicatively relevant observation to the listener is consistent with a view of generics as an essentially non-verbal but communicative phenomenon [@Csibra2015].
...
L_0(r, \mid u = gen) &= \frac{P(u = gen \mid r) \cdot P(r)}{\int_0^1 {P(u = gen \mid r') \cdot P(r') \diff r'}}  \nonumber \\
 &= \frac{P(x = 1 \mid n = 1; r) \cdot P(r)}{\int_0^1 {P(x = 1 \mid n = 1l r') \cdot P(r') \diff r'}}  \nonumber \\
...
The generic interpreation model used by @Tessler2019psychrev makes the strong claim that the literal contribution of a generic sentence is informationally equivalent to observing a single, positive example.
...
The prevalence prior can be viewed as a distribution over different categories (and their associated prevalence of the feature).
...
In Experiment 1, we measure the parameters of this mixture-distribution in order to measure beliefs about relatively obscure properties.
