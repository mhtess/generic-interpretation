...
We replicate this result and extend it to other types of biological properties. 
This experiment serves to validate our measurement, which we will use in the main experiments. 
...
As in the replication of @Cimpiain2010, we observe a clear gradient in the implied prevalence ratings across our seventy-five items (Figure\ \@ref(fig:genint-empiricalData)).
...
Our primary analyses ask how well the various semantics models can accmmodate the human generic interpretation. 
We do this using a joint Bayesian inference method, wherein we infer the latent values of the variables governing the prevalence priors from the prevalence prior data (Expt. 1a) and the generic interpretation data (Expt. 1b). 
That is, we ask is there some reasonable setting of the prevalence priors that, together with a particular model of generic interpretation, could give rise to the generic interpretation results. 
To preview our results, we consistently find that the single observation / uncertain threshold model accomodates the generic interpretation results, while models that update beliefs based on a fixed value of a threshold (e.g., "some" or "most") are misspecified. 
...
For each candidate model of generic interpretation (literal, pragmatic, "some", "most"), we build a joint Bayesian data-analytic to simultaneously model the prevalence prior data (Expt. 1a) and the generic interpretation data (Expt. 1b), similar to the methods of @Tessler2019. 
...
The inferred prevalence prior distributions are then used as $P(r)$ in whatever model of generic interpretation we are testing in order to generate predictions for the generic interpretation data.^[
  In reality, this procedure is done simultaneously, not sequentially.
]
In addition, the pragmatic generic interpretation model (Eq. \ref{eq:L1}) has two additional free parameters that we infer from the data: the rationality of the speaker model $\alpha$ and the speaker's cost of producing the generic utterance (in comparison to staying silent) $c = \text{cost}(generic)$.
We put uninformative priors, consistent with the literature on this class of computational models, over these parameters: $\alpha \sim \text{Uniform}(0, 30)$ and $c \sim \text{Uniform}(0, 10)$.
The literal generic interpretation model and models of "some" and "most" quantifier semantics have no free parameters.
To learn about the credible values of the parameters as well as generate model predictions for the generic interpretation data, we performed Bayesian inference on the model by running three chains of an incrementalized version of MCMC [@Ritchie2016] for 750,000 iterations, removing the first 250,000 iterations for burn-in.
Convergence was checked qualitatively by confirming the similarity of the results for each chain; there were no appreciable differences of results between chain.

#### Model criticism

A qualitative examination of the inferred prevalence prior distributions showed that they well-modeled as a mixture of three Beta distributions. 
...
To evaluate how well each model can accomodate the implied prevalence data, we compare the Maximum A-Posteriori value of each model's posterior predictive distribution of implied prevalence ratings to the mean implied prevalence ratings for each of the 75 items (Figure\ \@ref(fig:genint-modelingResults)).
Table\ \@ref(tab:genint-modelTable) shows the mean squared errors and proportion of explained variance for predictions based on the prevalence prior alone, a literal "some" model, a literal "most" model (which includes an additional fit noise parameter), a literal generics (single positive observation) model, and the pragmatic generics model. 
We see that the models based on the prevalence prior and the quantifier "some" (i.e., a fixed threshold at the lowest possible value) consistently *underpredict* the implied prevalence, consistent with the intuition that assigning the semantics of "some" for generics is too weak.
Though the model based on a semantics for "most" (i.e., a fixed thrshold at 0.5) does better in terms of mean squared error, it explains substantially less variance; the "most" model assigns `r format_empirical_ci(m.samp.most.noise, "MAP", "cred_lower", "cred_upper")` proportion of the data to be noise, because of the high rate of implied prevalence ratings below 50\%. 
...
Unlike each of the alternative semantic models, the two models based on belief updating according to a single, positive observation (literal and pragmatic generics models) can accomodate the highly variable generic interpretation data with a high degree of quantitative accuracy, with the pragmatic model slightly outperforming the literal generics model.
...
Figure\ \@(fig:genint-modelingResults-bars) shows empirical means and model predictions for ten example items covering the range of responses. 
Both the literal and pragmatic generic interpretation models track the quantitative variance across the items.
The pragmatic model model provides a stronger quantitative fit for items that receive high but not at-ceiling implied prevalence ratings (e.g., *have spots*, *hunt other animals*). 
...
Our second data analytic strategy is a formal model comparison to determine if the improved quantitative fit exhibited by the pragmatic generic interpretation model is a meaningful difference and justifies its use of additional free parameters.
To do this, we compute Bayes Factors which compare the likelihood of the observed data under each of the models, averaging across all values of the free parameters in the model. 
...
Comparing our models using a fully Bayesian strategy with uncertainty over the values of the parameters of prevalence priors (analogous to what is performed in the model criticism section) is computationally challenging. 
Therefore, we compute the likelihood of the data under each model by fixing the prevalence priors to be the raw empirical distributions; fixing the prevalence priors reduces the overall flexibility of the models, but it does so in a way that penalizes all models to an equal extent.
To deal with the additional flexibility of the pragmatic model afforded by its two additional free parameters, we used an Annealed Importance Sampling algorithm [@Neal2008] implemented in the probabilistic programming language WebPPL to calculate the marginal likelihood of the data under the pragmatic model. 
The results are shown in Table 1: The pragmatic model is on the order of $e^{60}$ times better at accounting for the implied prevalence data, even taking into the model's slight additional complexity. 
...
```{r genint-empiricalData, fig.width = 11, fig.asp = 0.95, fig.cap="Implied prevalence ratings for a broad set of seventy-four items. Vertical lines denote means, points are individual empirical judgments. Error bars denote bootstrapped 95% confidence intervals.", cache = F}
...
example.items <- c("have spots", 
                   ...
                   #"eat insects", 
                   "chase their tails",
                   "ride the subway","perform in the circus", "drink soda",
                   "hunt other animals", "have four legs",
                   "carry Lyme disease", "live to be 100 years old")
...
md.models.bars <- md.litprag.long %>%
      filter(parameter == "posterior" | parameter == "data" | src == "pragmatic_unlifted", 
             ...
             !(src =="pragmatic")) %>%
  mutate(
    src = paste(src, semantics, parameter, sep = "_"),
...
In the previous experiment, we saw how participants' prior beliefs about the prevalence of the property measured by asking about alternative categories (Expt. 1) related to the prevalence implied by a novel generic via a model that reasons about an uncertain threshold on prevalence (Eq. \ref{eq:L0}).
