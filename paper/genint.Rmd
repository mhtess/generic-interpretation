...
The premier example of language's ability to transmit abstract knowledge comes in the form of statements that convey generalizations, otherwise known as *generic language* (e.g., "Birds fly"). 
Generics are ubiquitous in everyday conversation and child-directed speech [@Gelman1998].
Unlike statements about concrete events or exemplars  (e.g., "This bird flys"), generics are not directly observable and are, in fact, resilient to counterexamples (e.g., penguins do not fly). 
Generics are also distinct from quantified statements: "Some students failed the test" implies that at least one student failed the test (as well as pragmatically implicating that "not all" failed the test), "Most dogs bark" implies that at least %50 of dogs bark; "All triangles have three sides" means that if a shape has four sides, it is not a triangle.
3-year-olds endorse novel generics differently from quantifiers [@Gelman2015genericsQuantifiers], and 4-year-olds interpret novel generics about biological properties of animals (e.g., "Crullets have spots") as implying a near-universal [e.g., almost all crullets have spots; @Gelman2002; @Brandone2014].

Interpretations of generic statements are also sensitive to the kind of property predicated.
Intuitively, "Mosquitos carry malaria" means that *some mosquitos* carry malaria, while "Mosquitos have wings" implies something stronger: *most or all mosquitos* have wings.
Empirically, context-sensitive interpretations have been shown for properties that are biological in nature (e.g., *have wings*) vs. accidental [e.g., *have fungus-covered claws*; @Cimpian2010].


The flexibility of generic interpretation is in part what makes it so intruiging. 
Why should language that is so central to learning have such an uncertain meaning?
Some have proposed that theory-based expectations guide our understanding of generic statements [@Leslie2008; @Cimpian2010theory; @Prasada2013], but these ideas have not been made sufficiently precise to make quantative predictions.

Recently, @TesslerLangGenUnderReview proposed a model of generic language based on an underspecified threshold criterion (e.g., that generics are like quantifiers with a vague, uncertain threshold).
This model was shown to account for truth judgments across a wide range of linguistic stimuli including generics about animals, habituals about actions (e.g., "John runs"), and causal language (e.g., "This herb makes animals sleepy"). 
The model formalized endorsement as a speaker decision about whether or not to say a generic sentence to a naive listener, which can interpret generic sentences. 
To interpret a generic sentence, the listener model draws upon abstract knowledge about properties, formalized by a prior distribution over likely prevalence levels. ^[
  Though not the direct focus of this work, the prevalence prior distribution can be seen as a shallow formalization of theory-based considerations. That is, different theory-based relationships between kinds and properties will give rise different prior distributions over prevalence, which acts as a statistical layer between conceptual knowledge and natural language semantics. 
]
In this paper, we test this model of generic interpretation. 

The paper is organized as follows.
First, we reintroduce the computational model of generic interpretation. 
...
Together, these results suggest that a quantitative understanding of generic language.
