...
As is shown in Table 4, the pragmatic generics model is by far the best model at predicting these data.
Thus, the prevalence prior is causally related to interpretations of novel generics in the way predicted by the pragmatic generics model.
...
load("cached_results/genint_manipPriors_bf2.RData") # model.likelihoodRatios.summarized
...
md.priorManip.summary.table <-left_join(
  ...
  model.likelihoodRatios.summarized %>%
    ...
    mutate(model = ifelse(model == "uncertain", "literal", model))
  ) %>%
          filter(model %in% c("literal", "pragmatic_unlifted",
                          "prior", "fixed_0.05", "fixed_0.125", "fixed_0.375")) %>%
    mutate(`Model` = factor(model, levels = c("prior", "fixed_0.05",
                                              "fixed_0.125", "fixed_0.375", "literal",
                                              "pragmatic_unlifted"),
                                    labels = c("Prior", 
                                               "Threshold = 0.01",
                                               "Threshold = 0.1",
    ...
    mutate(`log Bayes Factor` = ifelse(Model == "Pragmatic Generic", "0", `log Bayes Factor`)) %>%
