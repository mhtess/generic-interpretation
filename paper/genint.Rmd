...
What sort of literal meaning could account for the panoply of interpretations of generic sentences?

If in fact "Mosquitos carry malaria" conveys something analogous to *some mosquitos, under some circumstances, carry malaria* then the knowledge conveyed by the generic could be similar to learning (i) one mosquito, in one particular circumstance, was once observed carrying malaria and (ii) given that properties covary with kinds in systematic way, if one mosquito once had this property, then it is reasonable to think there is some larger (but unknown) subset of mosquitos in some larger (but unknown) class of situations, also carry malaria.
Thus, one intuitively appealing model of understanding a generic statement would be equivalent to Bayesian belief-updating of structured prior knowledge [@Tenenbaum2011] after observing a single positive example of the category with a property.
Even with sufficiently structured prior knowledge about categories and properties, however, the information conveyed by a generic may be more than just a single observation.
One plausible explanation is that stronger interpretations are the result of pragmatic enrichment of the literal content of generics. 
...
Such a computational model is highly consistent with a view of generics as an essentially non-verbal but communicative phenomenon [@Csibra2015].


In this paper, we propose a model for learning from generic langauge that takes a generic statement to literally that a single instance of the category has the property.
We show that this model is mathematically equivalent to the interpretation model proposed by @TesslerLangGenUnderReview, wherein generics were conceived as a kind of vague quantifier.
...
We test these models to additional control models that use the same prior knowledge but which update their beliefs via different semantic mechanisms (e.g., fixed-thresholds) in three experiments. 
In Experiment 1, we replicate and extend the findings of @Cimpian2010 that biological and accidental properties receive differential interpretations and show that these generic interpretation models are able to capture the gradience in interpretation judgments; the observed variability cannot be predicted by simple models of quantifiers, even when coupled with substantive prior knowledge.
...
<!-- Similarily, inferences about members of a category can be quite weak when the property is not expected to be common [e.g., "Leo is a lion. Thus, Leo eats people."; ]. -->
<!-- Though intuitively generics can have *weak implications*, the actual implied prevalence from weak generics in these previous studies was still quite high [in @Cimpian2010 Expt. 3, mean implied prevalence $= 70\%$], leaving open the question of whether bonafide weak interpretations (e.g., implied prevalence $<50\%$) can result from interpreting novel generics.  -->
