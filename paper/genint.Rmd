...
We take the conditional probability of the generic utterance $u$ given some prevalence $r$---$P(u\mid r)$---to represent the literal semantic content of the generic utterance [@Frank2012], which we define to be belief-updating from a single positive observation:$P(x =1 \mid r; n = 1)$. 
...
\noindent since $P(\theta) \propto 1$. To arrive at $L(r \mid u)$ from $L(r, \theta \mid u)$, we integrate out $\theta$.
...
<!-- \end{array} -->
\begin{align}
L_0(r, \mid u) &= \int_{0}^{1} P(r, \theta \mid u) \diff \theta \nonumber \\
...
Equation \ref{eq:S1} is the endorsement model of @TesslerLangGenUnderReview, where an approximately-rational speaker decides whether or not to produce a generic utterance to convey information to a literal listener (Equation \ref{eq:L0}).
...
Under the interpretation of a generic sentence as communicating a single, positive observation, the alterantive is the *not* producing that observation. 
...
Both the literal and the pragmatic generic interpretation models (Equations \label{eq:L0} and \label{eq:L1}, respectively) balance the prior probability of different prevalence levels $P(r)$ with the overall preference for higher prevalence levels $r$.
Thus, interpretations strongly depend on the prevalence prior $P(r)$.
...
Interpretations from the literal generic interpretation model (Figure\ \@ref(fig:modelSimulations)B) are similar to different quantified statements for different properties in question.
...
In Experiment 2, we directly manipulate the prevalence prior.
...
Listeners may derive a stronger interpretation of a generic, however, by reasoning about the communicative nature of the sentence they hear [@Grice1975; @Clark1996].
That is, a generic sentence is produced by a speaker who has the goal of conveying information to the listener. 
The rightmost column of Figure\ \@ref(fig:modelSimulations)B shows the predictions of Eq. \ref{eq:L1}.
...
For the other prevalence priors, the pragmatic model makes stronger predictions by predicting lower-variance interpretations.
...
In order to assess the predictive capabilities of the generic interpretation models, we construct a number of alternatives for comparison.
Foremost, we compare the generic interpretation models to a model that does not update beliefs and predicts prevalence ratings according to the prior distribution $P(r)$. 
...
In addition, we construct two alternatives models to elucidate the belief-updating claims of the generic interpretation model; these alternative models use fixed-threshold functions to update beliefs, corresponding to models of quantifier semantics.^[
...
fig.sims.distributions <- ggplot(sims.combined %>%
         ...
         mutate(PriorShape = factor(PriorShape, levels = c("uniform", "biological_rare", "sexed_rare", "accidental_rare"),
                ...
                src = factor(src, levels = c( "fixed_0.1", "fixed_0.33",
                                              "fixed_0.5", "literal", "pragmaticUnlifted"),
                             labels = c(
                           #    'prevalence prior',
                               '"some"\n(threshold = 0.01)',
                               '"more than a third"\n(threshold = 0.33)',
    ...
    scale_color_manual( values =  c(spectrum.color.palette[c(9, 6, 3)], "#99d8c9", "#2ca25f"))+
    scale_fill_manual( values = c(spectrum.color.palette[c(9, 6, 3)], "#99d8c9", "#2ca25f") )+
    facet_grid(PriorShape~src, scales = 'free')+
...
We showed how this model is mathematically equivalent to the interpretation model assumed by @Tessler and we articulated a more elaborate model which understands generics as intentional utterances.
...
Furthermore, we found that a model that interprets generics pragmatically provides a better explanation of the observed data.
...
The fact that the literal meaning of a generic is equivalent to observing a single positive instance of the category with a property and that interpretation is best modeled as a communicatively relevant observation suggests a very primitive foundation to generic language. 
Bayesian belief-updating based on observations is a plausible learning mechanism for any animal \red{(is there some citation for bayes in other animals?)}.
...
@Csibra2015 argues that individual objects are interpreted by infants as symbolic representations of the object kind.
In that way, it is possible to interpret a pedagogically presented exemplar from a category as a "nonverbal generic".
The pedagogical example is thus an indexical that refers to the category. 
...
A speaker might endorse a generic when the prevalence is quite low [see @TesslerGoodmanInPress] believing that they and the listener share the background assumptions that the property is a low-prevalence property in general. 
