  ...
  slides.prior_manipulation = slide({
   ...
   // present : _.shuffle(_.range(numTrials)),
    ...
    nextExperiment: function() {
      ...
      console.log("waitFlag = " + this.waitFlag)
      $("#h" + i).show()
      $("#d" + i + "a").show()
      $("#d" + i).show()

      $("#h" + i).html("% " + this.experimentNames[i] + 's')
      // $("#h" + i).html("% " + this.experimentNames[i].category)
      // $("#h" + i).css({"font-size":"13px", "border":"1px dotted black"})
      $("#d" + i).css({"padding":"10px", "font-size":"18px","font-weight":"bold", "border":"1px solid black"});
      ...
      var experimentEvent = 'You discover a new animal, which you call Animal ' + this.experimentNames[i] + ".<br><br>";
      // 'There is an animal on the planet called '+
      // this.stim.categories[i].category + ".<br><br>";//'The scientists studied 100 ' + this.stim.categories[i].category + " in their natural habitat over the course of the past year.";
      ...
      if (this.stim.data[i] == "?") {
        ...
        $("#evidenceDescription").html(experimentEvent + "" + experimentResults)
      ...
      if (this.count == (this.experimentNames.length - 2)) {
        ...
        if (this.waitFlag == 1) {
          ...
          if (exp.condition == "prior"){
            ...
            // utt = '<strong>Your partner tells you</strong>:<br><br><div style="text-align:center; font-size:26px"><strong>"' + jsUcfirst(this.stim.categories[this.missing].category) + " " + this.stim.property +'."' + '</strong></div><br>'
            utt = '<strong>Your partner tells you</strong>:<br><br><div style="text-align:center; font-size:26px"><strong>"' + this.experimentNames[this.missing] + "s " + this.stim.property +'."' + '</strong></div><br>'
            ...
            setTimeout(function(){
                // console.log(_s.stim)
                // console.log(_s.missing)
                $("#nextExptButton").hide();
                utt += '<br><br>What percentage of ' +_s.experimentNames[_s.missing] + 's do you think <i>' + _s.stim.property + "</i>?";
...
function init() {
  ...
  repeatWorker = false;
  (function(){
      var ut_id = "mht-intmanip-20170417";
  ...
  var creatures = _.map(shuffledCreatures.slice(0,exp.n_data + 1),
    function(x){return {category: x.category, exemplar: x.exemplar}}
    )

  exp.nextExperimentNames = _.map(shuffledCreatures.slice(exp.n_data + 1, exp.n_data + 5),
    function(x){return {category: x.category, exemplar: x.exemplar}}
    )
