...
model.prefix <- "pilot-results-genint-L0-inferNComponents_"
n_samples <- 100
n_burn <- n_samples / 2
lg <- 2
chain <- 1
...
for (modfile in model.files){
  ...
  m.samp <- bind_rows(
    m.samp,
    rwebppl::get_samples(m.item, n_samples)
  )
...
```{r}
m.samp %>%
  filter(parameter == "numberOfComponents") %>%
  group_by(property, val) %>%
  count()
```
...
```{r}
m.samp.iter <- m.samp %>%
  filter(parameter %in% c("componentParameters", "numberOfComponents")) %>%
  mutate(parameter = paste(parameter, property, componentID, componentParam, sep = "_")) %>%
  select(-property, -componentID, -componentParam) %>%
  group_by(parameter) %>%
