---
title: "Analysis of Experiments 1: Prior elicitation and Generic interpretation tasks"
...
```{r}
df.int.filtered.bs.norm.wide <- left_join(
  df.int.filtered.bs,
  df.int.filtered.bs.norm %>%
    rename(n_norm = n, mean_norm = mean, lower_norm = ci_lower, upper_norm = ci_upper) %>%
    select(-empirical_stat)
)

print(with(df.int.filtered.bs.norm.wide, cor(mean_norm, mean)))

df.int.filtered.bs.norm.wide %>%
  ggplot(., aes( x = mean_norm, xmin = lower_norm, xmax = upper_norm,
...
## Comparison of full distributions

```{r}
bind_rows(
  df.int.filtered %>%
    mutate(src = 'original') %>%
    select(property, src, response),
  df.int.rep.filtered %>%
    mutate(src = 'replication') %>%
    select(property, src, response)
) %>%
...
ggsave("ecdf_genint_genintrep.pdf", width = 14, height = 20)
```
...
df.int.filtered %>%
  ...
  write_csv("../data/paper_data/expt1/expt1b-explanations.csv")
