...
model.path <- "../models/results/bootstrap/"
# ran L0 and L1 model likelihood simulations serparately
model.prefixes <- c(
...
for (model.prefix in model.prefixes){
  for (sfpp in sample_from_prior_probs){
    ...
    for (modfile in model.files){
      ...
      # computed model likelihoods for prior in each file, only need 1
      m.item <- if (model.prefix == "generics-bda-L1unlifted-btstrpPriors-likelihood-results_sampleFromPriorProb") {
        m.item %>% filter(model != "prior")
      } else { m.item }
...
```{r bootstrap interpretation distributions (summarize), eavl = F}
