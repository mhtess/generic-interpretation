...
# FULL DATA COLLECTION: Prior manipulation 3 interpretation

```{r load catch data pm int 3}
...
df.prior_manip.int.filtered.n_subj_per_item <- df.pm.int.3.filtered %>%
  group_by(distribution) %>%
...
md.pm.int.3.filtered.expvals.bs.summary <- bind_rows(
...
md.pm.int.3.filtered.expvals.bs.summary %>% 
  ungroup() %>%
...
md.pm.int.3.filtered.expvals.bs.summary.wide <- left_join(m.preds.zeroPadded.expectations %>%
    group_by(model, distribution) %>%
...
df.pm.int.3.filtered.prior_mean <- left_join(
  df.pm.int.3.filtered,
  md.pm.int.3.filtered.expvals.bs.summary %>%
    filter(model == "prior") %>%
...
rs.lm.priorManip <- lm(data = df.pm.int.3.filtered.prior_mean,
     response ~ prior_mean)
...
rs.lm.priorManip.contrast1 <- lm(
  data = 
    df.pm.int.3.filtered.prior_mean %>%
    filter(
...
df.pm.int.3.filtered.prior_mean$distribution <-  factor(df.pm.int.3.filtered.prior_mean$distribution,
                                                        levels = contrast1.levels)
custom_contrasts <- MASS::contr.sdif(10)
colnames(custom_contrasts) <- paste("a", seq(1, 9))
contrasts(df.pm.int.3.filtered.prior_mean$distribution) <- custom_contrasts
...
contrasts(df.pm.int.3.filtered.prior_mean$distribution) <- cbind(c1 = c(-2,1,1,0,0,0,0,0,0,0),
...
rs.brm.priorManip.sdif <- brm(
  formula = response ~ d,
  data = df.pm.int.3.filtered.prior_mean %>% rename(d = distribution),
...
contrasts(df.pm.int.3.filtered.prior_mean$distribution)
```
