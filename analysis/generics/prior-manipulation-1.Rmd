---
title: "Prior manipulation pilots"
...
#### bootstrap priors (check that this works)
...
```{r bsDistributionsAndExpvals2, fig.width = 20, fig.height = 8}
m.preds.zeroPadded %>%
  #filter(threshold %in% c("prior", "generic")) %>%
...
m.preds.zeroPadded.expectations %>%
  ...
  ungroup() %>%
  mutate(distribution = factor(distribution, levels = property.order)) %>%
  ggplot(., aes( x = model, y = mean, ymin = lower, ymax = upper, fill = distribution))+
