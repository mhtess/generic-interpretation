...
model.likelihoods.summarized <- model.likelihoods %>%
...
model.likelihoods.summarized %>%
  mutate(model = factor(model, levels = model[with(model.likelihoods.summarized, order(mean_likelihood))])) %>%
...
ggsave("figs/prior-manipulation-3-model-likelihoods.pdf", width = 6, height = 4)
```


```{r bayes factors}
model.likelihoodRatios.summarized <- model.likelihoods %>%
  # filter(distribution %in% c("rare_weak", "rare_strong",
  #                            "rare_half", "rare_deterministic")) %>%
  group_by(model, iteration) %>%
  summarize(sum_likelihood = sum(likelihood)) %>%
